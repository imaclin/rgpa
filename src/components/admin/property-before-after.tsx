...
export function PropertyBeforeAfter({ propertyId }: { propertyId: string }) {
  ...
  const uploadFile = async (file: File, prefix: string): Promise<string | null> => {
    ...
    const fileName = `before-after/properties/${propertyId}/${prefix}-${Date.now()}.${fileExt}`
  ...
  const handleReorder = async (pairId: string, direction: "up" | "down") => {
    ...
    const supabase = createClient()
    await Promise.all([
      supabase.from("property_before_after").update({ display_order: newPairs[idx].display_order }).eq("id", newPairs[idx].id),
      supabase.from("property_before_after").update({ display_order: newPairs[swapIdx].display_order }).eq("id", newPairs[swapIdx].id),
    ])
  }

  const handleDelete = async (pair: Pair) => {
    const supabase = createClient()
    const { error } = await supabase.from("property_before_after").delete().eq("id", pair.id)
    if (error) {
  ...
  const handleAddPair = async () => {
    ...
    const { error } = await supabase.from("property_before_after").insert({
