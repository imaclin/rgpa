...
import { PropertyFiles } from "@/components/admin/property-files"
import { PropertyBeforeAfter } from "@/components/admin/property-before-after"
...
export function PropertyMedia({ propertyId, media, onMediaChange }: PropertyMediaProps) {
  ...
  const handleDragEnd = async () => {
    ...
    // Update sort_order in database if we have a propertyId
    if (propertyId) {
  ...
  const handleUpload = async (files: File[]) => {
    ...
    for (const file of files) {
      ...
      if (propertyId) {
        ...
        // For new properties, just store locally
        newMedia.push({
          url: publicUrl,
          alt_text: file.name,
          type: file.type.startsWith("video/") ? "video" : "image",
    ...
    // If this is the first image and we have a propertyId, set it as featured
    if (propertyId && media.length === 0 && newMedia.length > 0 && newMedia[0].type === "image") {
      const supabase = createClient()
      await supabase
        .from("properties")
    ...
    <Card>
      ...
      <Tabs defaultValue="media" className="space-y-4">
        ...
        <TabsContent value="files">
          {propertyId ? (
            ...
            <div className="py-8 text-center text-sm text-muted-foreground">
              Save the property first to upload files.
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="before-after">
          {propertyId ? (
            <PropertyBeforeAfter propertyId={propertyId} />
          ) : (
            <div className="py-8 text-center text-sm text-muted-foreground">
              Save the property first to add before/after pairs.
            </div>
          )}
        </TabsContent>
      </Tabs>
