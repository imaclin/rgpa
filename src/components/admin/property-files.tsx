...
export function PropertyFiles({ propertyId }: PropertyFilesProps) {
  ...
  const [deletingFile, setDeletingFile] = useState<PropertyFile | null>(null)
  ...
  const getPreviewType = (file: PropertyFile): "pdf" | "image" | "office" | "text" | "none" => {
  ...
  const handlePreview = async (file: PropertyFile) => {
  ...
  const handleDelete = async () => {
    ...
    const { error } = await supabase.from("property_files").delete().eq("id", deletingFile.id)
