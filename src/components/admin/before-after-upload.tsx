...
export function BeforeAfterUpload() {
  ...
  const uploadFile = async (file: File, prefix: string): Promise<string | null> => {
    ...
    const fileName = `before-after/${prefix}-${Date.now()}.${fileExt}`
  ...
  const handleReorder = async (pairId: string, direction: "up" | "down") => {
    ...
    const supabase = createClient()
    await Promise.all([
      supabase.from("before_after_pairs").update({ display_order: newPairs[idx].display_order }).eq("id", newPairs[idx].id),
      supabase.from("before_after_pairs").update({ display_order: newPairs[swapIdx].display_order }).eq("id", newPairs[swapIdx].id),
    ])
  }

  const handleDelete = async (pair: Pair) => {
    const supabase = createClient()
    const { error } = await supabase.from("before_after_pairs").delete().eq("id", pair.id)
    if (error) {
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>Before & After Pairs</CardTitle>
              <p className="mt-1 text-sm text-muted-foreground">
                Manage before/after comparisons shown on the homepage
              </p>
            </div>
        ...
        <CardContent className="space-y-6">
          ...
          {pairs.length === 0 && !isAdding && (
            <div className="py-8 text-center text-muted-foreground">
              ...
              <p className="mt-3 font-medium">No before/after pairs yet</p>
              <p className="mt-1 text-sm">Click &ldquo;Add Pair&rdquo; to create your first comparison</p>
            </div>
          )}
                ...
                <div className="flex items-center gap-2">
                  <div className="flex flex-col">
                    <Button
                      ...
                      className="h-6 w-6"
                      disabled={idx === 0}
                      onClick={() => handleReorder(pair.id, "up")}
                    >
                    ...
                    <Button
                      ...
                      onClick={() => handleReorder(pair.id, "down")}
                    >
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-2">
                  <div className="relative flex aspect-video cursor-pointer items-center justify-center overflow-hidden rounded-lg border-2 border-dashed border-border hover:border-muted-foreground/50"
                    onClick={() => {
                      setReplaceTarget({ id: pair.id, type: "before" })
                      replaceBeforeRef.current?.click()
                    }}
                  >
                ...
                <div className="space-y-2">
                  <div className="relative flex aspect-video cursor-pointer items-center justify-center overflow-hidden rounded-lg border-2 border-dashed border-border hover:border-muted-foreground/50"
                    onClick={() => {
                      setReplaceTarget({ id: pair.id, type: "after" })
                      replaceAfterRef.current?.click()
                    }}
                  >
            ...
            <div className="rounded-lg border border-dashed border-navy p-4 space-y-4 dark:border-[#3b82f6]">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <div
                    className="relative flex aspect-video cursor-pointer flex-col items-center justify-center overflow-hidden rounded-lg border-2 border-dashed border-border hover:border-muted-foreground/50"
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (file) handleNewFileSelect(file, "before")
                      e.target.value = ""
                    }}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const file = e.target.files?.[0]
                      if (file) handleNewFileSelect(file, "after")
                      e.target.value = ""
                    }}
                  />
                </div>
      ...
      {/* Hidden file inputs for replacing images */}
      <input
        ref={replaceBeforeRef}
        type="file"
        accept="image/*"
        className="hidden"
        onChange={(e) => {
          const file = e.target.files?.[0]
          if (file && replaceTarget) handleReplaceImage(replaceTarget.id, file, "before")
          e.target.value = ""
        }}
      />
      <input
        ...
        onChange={(e) => {
          const file = e.target.files?.[0]
          if (file && replaceTarget) handleReplaceImage(replaceTarget.id, file, "after")
          e.target.value = ""
        }}
      />
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => deletingPair && handleDelete(deletingPair)}
            >
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
