...
export function PropertyGallery({ media, propertyTitle }: PropertyGalleryProps) {
  ...
  // Preview layout: 1 large main image + 2 side images
  const mainImage = media[0]
  const sideImages = media.slice(1, 3)

  return (
    <>
      {/* Gallery Preview Grid */}
      <div className="relative grid gap-2 overflow-hidden rounded-xl md:grid-cols-[2fr_1fr]">
        {/* Main Image - Left Side */}
        <button
          onClick={() => openModal(0)}
          className="relative aspect-[4/3] overflow-hidden bg-muted focus:outline-none focus:ring-2 focus:ring-primary md:aspect-auto md:h-full md:min-h-[28rem]"
        >
        ...
        <div className="hidden gap-2 md:grid md:grid-rows-2">
          {sideImages.map((item, index) => (
            <button
              ...
              className="relative overflow-hidden bg-muted focus:outline-none focus:ring-2 focus:ring-primary"
            >
              {item.type === "image" ? (
                <img
                  src={item.url}
                  alt={item.alt_text || propertyTitle}
                  className="h-full w-full object-cover transition-transform hover:scale-105"
                />
              ) : (
                <video
          ...
          >
            {media[selectedIndex].type === "image" ? (
              <img
                ...
                alt={media[selectedIndex].alt_text || propertyTitle}
