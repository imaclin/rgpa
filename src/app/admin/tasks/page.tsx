...
export default function TasksPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (propertiesData) {
      ...
      // Fetch task counts per property
      ...
      if (tasksData) {
        ...
        for (const task of tasksData) {
          if (!summaries[task.property_id]) {
            summaries[task.property_id] = { property_id: task.property_id, total: 0, completed: 0, overdue: 0 }
          }
          summaries[task.property_id].total++
          if (task.status === "completed") {
            summaries[task.property_id].completed++
          }
          if (task.due_date && task.due_date < today && task.status !== "completed") {
            summaries[task.property_id].overdue++
          }
        }
  ...
  const handleClick = (propertyId: string) => {
    router.push(adminPath(`/admin/tasks/${propertyId}`))
  }
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Create a property first to start managing tasks.
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            {filteredProperties.map((property) => {
              const summary = taskSummaries[property.id]
