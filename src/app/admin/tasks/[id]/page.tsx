...
export default function PropertyTasksPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [propertyRes, tasksRes] = await Promise.all([
      supabase.from("properties").select("id, title, location, featured_image_url").eq("id", propertyId).single(),
      supabase.from("tasks").select("*").eq("property_id", propertyId).order("sort_order", { ascending: true }),
    ])
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="font-serif text-2xl font-bold leading-tight">{property.title}</h1>
          {property.location && (
